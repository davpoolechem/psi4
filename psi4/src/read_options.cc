...
int read_options(const std::string &name, Options &options, bool suppress_printing) {
    ...
    **Cfour Interface:** Keyword translates into |cfour__cfour_spherical|. -*/
    ...
    options.add_str("SCF_TYPE", "PK", "DIRECT DF MEM_DF DISK_DF PK OUT_OF_CORE CD GTFOCK DFDIRJ DFDIRJ+COSX DFDIRJ+LINK DFDIRJ+SNLINK CFMM CFMM+COSX CFMM+LINK CFMM+SNLINK DFCFMM DFCFMM+COSX DFCFMM+LINK DFCFMM+SNLINK");
    ...
    if (name == "CPHF" || options.read_globals()) {
        ...
        /*- SCF Type
         -*/
        options.add_str("SCF_TYPE", "DIRECT", "DIRECT DF PK OUT_OF_CORE PS INDEPENDENT GTFOCK DFDIRJ+LINK DFDIRJ+COSX DFDIRJ+SNLINK CFMM+LINK CFMM+COSX CFMM+SNLINK DFCFMM+LINK DFCFMM+COSX DFCFMM+SNLINK");
